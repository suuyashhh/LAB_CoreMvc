...
namespace Lab_Mvc.Interfaces
{
    public interface IOtherExpense
    {
        ...
        Task<DTOOtherExpense> GetOtherExpenseById(long otherEx_id, int comId);
        Task<List<DTOOtherExpense>> GetDateWiseOthMaterials(string from_date, string to_date, int comId);
        Task SaveOtherExpense(DTOOtherExpense objOtherEx);
        Task EditOtherExpense(DTOOtherExpense objOtherEx, long otherEx_id);
        Task DeleteOtherExpense(long otherEx_id, int comId);
    }
}
